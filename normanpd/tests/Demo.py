...
urli="http://normanpd.normanok.gov/"
...
''' specila code nort used for storing pdf files
import requests
url=urls[0]
r=requests.get(url,stream=True)
with open('/home/vishnu/Desktop/normanpd/1.pdf', 'wb') as fd:
        fd.write(r.content)'''
...
# /filebrowser_download/657/2017-02-19%20Daily%20Incident%20Summary.pdf
...
#'Date / TimeIncident NumberLocationNatureIncident ORI
#Daily Incident Summary (Public)NORMAN POLICE DEPARTMENT
#NORMAN POLICE DEPARTMENTDaily Incident Summary (Public)
#þ
#if "ABCD" in "xxxxABCDyyyy":
#perl -i.bak -pe "s/\t/' 'x(8-pos()%8)/eg" file.py
#SELECT * FROM incidents ORDER BY RANDOM() LIMIT 5;
#r'\d/\d\d/\d\d\d\d\s\d:\d\d'
